  ...
  export const APIContextProvider = (props: Props) => {
    ...
    async function getResults(cluster?: number, prefecture?: string) {
      if (prefecture && (cluster === 0 || cluster)) {
        await axios.get(
          `${process.env.REACT_APP_DOMAIN_NAME}/?pref=${prefecture}&cluster=${cluster}`
        ).then((res) => setResults(res.data.results))
      } else if (cluster === 0 || cluster) {
          await axios.get(
          `${process.env.REACT_APP_DOMAIN_NAME}/?cluster=${cluster}`
        ).then((res) => setResults(res.data.results))
      } else if (searchPref.prefOfOrigin && searchPref.cityOfOrigin) {
         await axios.get(
          `${process.env.REACT_APP_DOMAIN_NAME}/?city=${searchPref.cityOfOrigin}`
        ).then((res) => setResults(res.data.results))
      } else {
        await axios.get(`${process.env.REACT_APP_DOMAIN_NAME}`, {headers: {'Content-Type': 'application/json'}}).then((res) => setAllData(res.data.results))
      }
    }
    ...
    const getClusterOfPref = useCallback(() => {
      ...
      return cluster;
    }, [searchPref.prefOfOrigin, searchPref.cityOfOrigin, allData]);
